...
func buildTestPortalIndex(portal *sreportalv1alpha1.Portal) *sourcechain.PortalIndex {
	return &sourcechain.PortalIndex{
		Main:   portal,
...
// fakeEnabledLister implements sourcechain.EnabledSourcesLister for tests.
...
var _ = Describe("SourceReconciler", func() {
	...
	Context("ReconcileDNSRecordsHandler", func() {
		It("should create DNSRecord for a portal and source type", func() {
			...
			handler := sourcechain.NewReconcileDNSRecordsHandler(k8sClient)
			rc := &reconciler.ReconcileContext[struct{}, sourcechain.ChainData]{
				Data: sourcechain.ChainData{
					Index: buildTestPortalIndex(portal),
					EndpointsByPortalSource: map[sourcechain.PortalSourceKey][]*endpoint.Endpoint{
	...
	Context("DeleteOrphanedHandler", func() {
		It("should delete DNSRecords for disabled sources", func() {
			...
			deleteHandler := sourcechain.NewDeleteOrphanedHandler(k8sClient, &fakeEnabledLister{types: nil})
			deleteRC := &reconciler.ReconcileContext[struct{}, sourcechain.ChainData]{
				Data: sourcechain.ChainData{
					Index:                   buildTestPortalIndex(portal),
					EndpointsByPortalSource: make(map[sourcechain.PortalSourceKey][]*endpoint.Endpoint),
				},
			}
