...
// Source kinds discovered natively by external-dns in this package.
const (
...
//
// NOTE: CommonSourceSpec.FQDNTemplate and CombineFQDNAndAnnotation are
// intentionally NOT propagated in PR1. external-dns v0.21 drives templating
// through Config.TemplateEngine (source/template), which is out of scope here;
// the goal is restoring rules/tls/service/gateway extraction. A DNS CR setting
// fqdnTemplate gets no templated FQDNs and no error — tracked for PR2.
...
func (c *EffectiveConfig) hash(kind registry.SourceType) string {
	...
	fmt.Fprintf(&b, "ns=%q;af=%q;lf=%q;it=%v;pi=%t;ph=%t;icn=%v;ihn=%t;itls=%t;irul=%t",
