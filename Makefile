...
test: manifests generate fmt vet setup-envtest ## Run tests.
	KUBEBUILDER_ASSETS="$(shell "$(ENVTEST)" use $(ENVTEST_K8S_VERSION) --bin-dir "$(LOCALBIN)" -p path)" go test $$(go list ./... | grep -v /e2e | grep -v /web/) -coverprofile cover.out

# TODO(user): To use a different vendor for e2e tests, modify the setup under 'tests/e2e'.
# The default setup assumes Kind is pre-installed and builds/loads the Manager Docker image locally.
# CertManager is installed by default; skip with:
# - CERT_MANAGER_INSTALL_SKIP=true
KIND_CLUSTER ?= sreportal-test-e2e
...
.PHONY: build-all
build-all: manifests generate proto build-web build ## Build everything (CRDs, proto, web, manager).
...
run: manifests generate fmt vet generate-certs ## Run a controller from your host (dev mode: UI served from filesystem).
	SREPORTAL_CONTROLLER_SA=system:serviceaccount:sreportal-system:sreportal-controller-manager \
...
run2: manifests generate fmt vet generate-certs2 ## Run a second controller from your host (dev mode: UI served from filesystem).
	SREPORTAL_CONTROLLER_SA=system:serviceaccount:sreportal-system:sreportal-controller-manager \
		go run ./cmd/main.go -log-level=debug --web-bind-address=:8091 --health-probe-bind-address=:9091 --webhook-port=9444 --webhook-cert-path=$(CERTSDIR) --config=./config/samples/test_config.yaml
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name sreportal-builder
	$(CONTAINER_TOOL) buildx use sreportal-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm sreportal-builder
	rm Dockerfile.cross
