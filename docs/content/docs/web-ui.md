...
- **Flow Explorer** — select a resource to see its direct incoming and outgoing flows in a butterfly layout, grouped by namespace; each flow shows a "last seen" indicator when a flow observer (Hubble/Prometheus) is available; click any connected service to navigate to it
